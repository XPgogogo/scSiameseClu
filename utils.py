...
if os.path.isdir('scSiameseClu/log') == False:
    os.makedirs('scSiameseClu/log')


logger.add('scSiameseClu/log/{}.log'.format(opt.args.name), rotation="500 MB", level="INFO")
logger.info(opt.args)
...
def setup():
    ...
    - sigma_value: sigma value for zinb loss
    - mu_value: lambda value for clustering guidance
    - delta_value: gamma value for propagation regularization
    - lr: learning rate
    Return: None
    ...
    logger.info('Seed {}'.format(opt.args.seed))

    # opt.args.seed = random.randint(1,3000)
    # setup_seed(opt.args.seed)
    # logger.info('Seed {}'.format(opt.args.seed))

    # 检查参数的赋值情况，后面等确定最优参数后，请将下面代码进行赋值与修改，并解除注释


    if opt.args.name == 'Shekhar_mouse_retina_raw_data':
        opt.args.n_clusters = 19
        opt.args.n_input = 100
        # opt.args.alpha_value = 0.8
        # opt.args.sigma_value = 2
        # opt.args.mu_value = 10
        # opt.args.delta_value = 1e3
        # opt.args.lr = 1e-4
        opt.args.lr = 0.00928709683073195
        opt.args.alpha_value = 0.4362756133764631
        opt.args.sigma_value = 21.55953260264966
        opt.args.mu_value = 25.71353098163661
        opt.args.delta_value = 684522.6961093206
        opt.args.seed = 486259


    # elif opt.args.name == 'Macosko_mouse_retina':
    ...
    elif opt.args.name == 'Young_human_kidney_counts':
        opt.args.n_clusters = 11
        opt.args.n_input = 100
        opt.args.alpha_value = 0.5
        opt.args.sigma_value = 5
        opt.args.mu_value = 50
        opt.args.delta_value = 1e3
        opt.args.lr = 5e-3
        opt.args.seed = 5


    elif opt.args.name == 'Grace_CITE_CBMC_counts_top2000':
        opt.args.n_clusters = 15
        opt.args.n_input = 100
        opt.args.alpha_value = 0.32
        opt.args.sigma_value = 2
        opt.args.mu_value = 100
        opt.args.delta_value = 1e3
        opt.args.lr = 1e-4
        opt.args.seed = 50


    elif opt.args.name == 'Sonya_HumanLiver_counts_top5000':
        opt.args.n_clusters = 11
        opt.args.n_input = 100
        opt.args.alpha_value = 0.25
        opt.args.sigma_value = 2
        opt.args.mu_value = 10
        opt.args.delta_value = 1e3
        opt.args.lr = 1e-4
    ...
    elif opt.args.name == 'Meuro_human_Pancreas_cell':
    ...
    elif opt.args.name == 'Tabula_Qs_Lung':
        opt.args.n_clusters = 11
        opt.args.n_input = 100
        opt.args.alpha_value = 0.1
        opt.args.sigma_value = 1
        opt.args.mu_value = 10
        opt.args.delta_value = 1e3
        opt.args.lr = 5e-4
        opt.args.seed = 5
    ...
    print("beta value   : {}".format(opt.args.sigma_value))
    print("lambda value  : {}".format(opt.args.mu_value))
    print("gamma value   : {:.0e}".format(opt.args.delta_value))
    print("sigma_value    : {:.0e}".format(opt.args.sigma_value))
    print("learning rate : {:.0e}".format(opt.args.lr))
    print("epoch         : {}".format(opt.args.epoch))
    print("n_input       : {}".format(opt.args.n_input))
    print("-----------------------------")
...
def load_graph_data(dataset_name, show_details=False):
    ...
    """
    load_path = "scSiameseClu/dataset/" + dataset_name + "/" + dataset_name
    feat = np.load(load_path+"_feat.npy", allow_pickle=True)
    label = np.load(load_path+"_label.npy", allow_pickle=True)
    adj = np.load(load_path+"_adj.npy", allow_pickle=True)
    sf = np.load(load_path+"_sf.npy", allow_pickle=True)
    if show_details:
        print("++++++++++++++++++++++++++++++")
...
def load_pretrain_parameter(model):
    ...
    Returns: model
    """
    pretrained_dict = torch.load('scSiameseClu/model_pretrain/{}_pretrain.pkl'.format(opt.args.name), map_location='cpu')
...
def model_init(model, X, y, A_norm):
    ...
    with torch.no_grad():
        _, _, _, sim, _, _, Z, _, _, _, _, _ = model(X, A_norm, X, A_norm)

    # calculate cluster centers
    acc, nmi, ari, f1, centers, prediect_y = clustering(Z, y)
    print("model_init:")
    print(acc,nmi,ari)

    return sim, centers, prediect_y

# the ZINB loss function from scDSC    
def zinb_loss(X, mean, disp, pi, scale_factor=1.0, ridge_lambda=0.0):
    eps = 1e-10
    scale_factor = scale_factor[:, None]
    mean = mean.detach().cpu().numpy() * scale_factor
    mean = torch.from_numpy(mean).to(opt.args.device) 
...
def dicr_loss(Z_ae, Z_igae, AZ, Z):
    ...
    #  DICR loss function from DCRN
    loss_dicr = L_N + L_F + opt.args.delta_value * L_R
...
def clustering(Z, y):
    ...
    return acc, nmi, ari, f1, model.cluster_centers_, cluster_id
